            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li ><a href="uslugaIzmena.php"><i></i> Izmena usluge</a></li>
                    </ol>
                </section>
                ...
                <section class="content">
                    ...
                    <div class="col-xs-12">
                        <?php
                            ...
                            if (isset($_GET["id"]) && !isset($_POST["nazivUsluge"]) && !isset($_POST["cenaUsluge"])) {
                                $idUsluge = $_GET["id"];
                                $upit = "SELECT ID, naziv, cena FROM usluga WHERE ID = ".$idUsluge;
                                if (!$u=mysqli_query($baza, $upit)) {
                                    echo "Dogodila se greška prilikom čitanja usluge.</div>";
                                    die();
                                } if (mysqli_num_rows($u)==0) {
                                    echo "Usluga za id ".$idUsluge." nije pronađena!</div>";
                                }
                                $red=mysqli_fetch_array($u);
                                $nazivUsluge = $red['naziv'];
                                $cenaUsluge = $red['cena'];
                            }

                            if (isset($_POST["idUsluge"]) && isset($_POST["nazivUsluge"])) {
                                $naziv = trim($_POST["nazivUsluge"]);
                                $cena = $_POST["cenaUsluge"];
                                $id = $_POST["idUsluge"];

                                if (isset($naziv) && isset($cena) && $naziv != '' && $cena != '') {
                                    $upit = "UPDATE usluga SET naziv='".$naziv."', cena=".$cena." WHERE ID = ".$id;
                                    if (mysqli_query($baza, $upit)) {
                                        echo "<p>Uspešno ste izmenili uslugu.</p>";
                                        echo "<br>";
                                    } else {
                        ...
                        ?>
                            ...
                            <div class="box box-primary">
                                <div class="box-header">
                                    <h3 class="box-title">Izmena usluge</h3>
                                </div><!-- /.box-header -->
                                <!-- form start -->
                                <form role="form" method="post" action="uslugaIzmena.php">
                                    <div class="box-body">
                                        <input name="idUsluge" class="form-control" id="idInput"
                                               value="<?php if(isset($idUsluge))
                                                   echo($idUsluge); ?>" type="hidden">
                                        <div class="form-group">
                                            <label for="nazivInput">Naziv</label>
                                            <input name="nazivUsluge" class="form-control" id="nazivInput"
                                                   value="<?php if(isset($nazivUsluge))
                                                       echo($nazivUsluge); ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="cenaInput">Cena</label>
                                            <input name="cenaUsluge" class="form-control" id="cenaInput"
                                                   value="<?php if(isset($cenaUsluge))
                                                                echo($cenaUsluge); ?>">
                                        </div>
